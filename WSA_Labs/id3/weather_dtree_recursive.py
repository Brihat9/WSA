...
def get_attribute_count_dict(df, num_records):
    ...
    '''
    # global HEADERS
    # HEADERS = df.columns.values.tolist()[1:-1]
...
def make_decision_tree(old_df, old_attribute, old_attribute_values, old_parent_node):
    ...
    # for old_attribute_value in selected_old_attribute_values:
        ...
        # can remove this ...
        # if old_attribute_value in NODE_DICT:
        #     ''' if old attribute value is in NODE_DICT '''
        #     new_parent_node = NODE_DICT[old_attribute_value]
        # else:
        #     ''' if old attribute value is not in NODE_DICT '''
        #     # new_parent_node = Dnode(attribute_value)
        #     new_parent_node = Dnode(attribute_value, parent_of_parent)
        ...
        if selected_new_attribute_values != -1:
            if selected_new_attribute in NODE_DICT:
                ''' if forth attribute is in NODE_DICT '''
                new_parent_node = NODE_DICT[selected_new_attribute]
            else:
                ''' if forth attribute is not in NODE_DICT '''
